...
def _load_robust_resnet(device: str, weights: str = "checkpoints/robust_resnet50.pth") -> torch.nn.Module:
    ...
    try:
        ...
        checkpoint = torch.load(
            weights, pickle_module=dill, map_location=device
        )
...
def build_classifier(classifier: str, device: str, weights: str = None) -> torch.nn.Module:
    ...
    if classifier == "robust50":
        model = _load_robust_resnet(device, weights=weights or "checkpoints/robust_resnet50.pth")
...
def build_classifier_and_transforms(
    clf: str, device: str, use_tfms: bool, weights: str = None
    ...
    """
    classifier = build_classifier(clf, device, weights=weights)
