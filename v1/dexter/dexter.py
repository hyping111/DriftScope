...
from .pipelines.sd_modified_att import generate_two_branches
#from .pipelines.sd_modified import generate_two_branches
...
class DEXTER:
    ...
    def __init__(self, config: RunConfig, device: str = "cuda") -> None:
        ...
        self.clip_orig_text_encoder = (
            CLIPTextModel.from_pretrained(self.clip_orig_text_encoder_name, subfolder=self.clip_orig_text_encoder_name_subfolder).to(self.device).half()
        )
        ...
        if config.load_type == 'weights':
            ...
            self.pipe_modified.unet.load_state_dict(weights, strict=config.modified_model_strict_load)
            print(f"Loaded UNet weights from {config.modified_model}")
        ...
        if getattr(config, "gradient_checkpointing", False):
            self.pipe_base.unet.enable_gradient_checkpointing()
            self.pipe_modified.unet.enable_gradient_checkpointing()
    ...
    def analyze_classifier(
        ...
        for outer_idx in range(cfg.topk):
            ...
            self.text_encoder = TextPipeline(translation_matrix=self.translation_matrix, n_soft_prompt=self.config.n_soft_prompt, clip_checkpoint=self.clip_orig_text_encoder_name, clip_subfolder_encoder=self.clip_orig_text_encoder_name_subfolder, clip_subfolder_tokenizer=self.clip_orig_tokenizer_name_subfolder)
            self.text_encoder = self.text_encoder.to(device).half()
            # self.text_encoder.clip.model.gradient_checkpointing_enable()  # this uses 1 more GB than not enabling it??
            # self.text_encoder.bert.model.gradient_checkpointing_enable()
            ...
            for inner_optim_step in range(cfg.optim_steps):
                ...
                latents_base, latents_modified,kk_base,kk_mod = generate_two_branches(
                    self.pipe_base, self.pipe_modified, cond, uncond, cfg, dim=dim, guidance_scale=g, seed=diff_seed,
                )
                # classifier forward
                ...
                if cfg.save_images:
                    img_base = latents_to_pil(latents_base.detach(), self.vae)
                    img_modified = latents_to_pil(latents_modified.detach(), self.vae)
                    img_base[0].save(
    ...
    def find_unique_masks(self, num_masks_to_find: int) -> str:
        ...
        while successful_runs < num_masks_to_find:
            ...
            self.text_encoder = TextPipeline(
            ...
            for inner_optim_step in range(cfg.optim_steps):
                ...
                # Save images if requested
                if cfg.save_images:#latents_to_pil(latents_base.detach().half(), self.vae.half())
            ...
            if final_word is not None:
                ...
                if word_str not in found_words:
                    ...
                    if cfg.save_images:
                        img_base = latents_to_pil(latents_base.detach().half(), self.vae.half())
                        img_modified = latents_to_pil(latents_modified.detach().half(), self.vae.half())
                        img_base[0].save(
                            os.path.join(
                                self.out_folder, 
