...
class CLIPTextEncoder(nn.Module):
    ...
    def __init__(self, checkpoint: str = "openai/clip-vit-large-patch14", subfolder_encoder: str = "", subfolder_tokenizer: str = ""):
        super().__init__()
        self.model = CLIPTextModel.from_pretrained(checkpoint, subfolder=subfolder_encoder)
        self.tokenizer = CLIPTokenizer.from_pretrained(checkpoint, subfolder=subfolder_tokenizer)
        self.sos_clip = (
...
class CLIPTextEncoderWithProjection(nn.Module):
    ...
    def __init__(self, checkpoint: str = "openai/clip-vit-large-patch14", subfolder_encoder: str = "", subfolder_tokenizer: str = ""):
        super().__init__()
        _full_model = CLIPTextModelWithProjection.from_pretrained(checkpoint, subfolder=subfolder_encoder)
        self.model = _full_model       # same as before
        self.text_projection = _full_model.text_projection  # nn.Linear(hidden, proj_dim, bias=False)
        self.text_projection.requires_grad_(False)  # freeze the projection layer just in case
    ...
    def forward(
        ...
        # EOS embedding is always the last token in the composed sequence
        # (see _compose_prompt — self.eos_clip is always appended at the end)
        pooled_output = last_hidden_state[:, -1, :]
        text_embeds = self.text_projection(pooled_output)  # shape: [batch, projection_dim]
