...
from ..models.clip_text_encoder import CLIPTextEncoder, CLIPTextEncoderWithProjection
from ..utils import PACKAGE_ROOT, make_translation_matrix_name


class TranslationMatrix(nn.Module):
    """Maps overlapping vocabulary entries between CLIP and BERT tokenizers."""

    def __init__(self, clip_checkpoint, bert_checkpoint, clip_subfolder_tokenizer="", bert_subfolder_tokenizer=""):
        super(TranslationMatrix, self).__init__()
        self.clip_tokenizer = CLIPTokenizer.from_pretrained(clip_checkpoint, subfolder=clip_subfolder_tokenizer)
        self.bert_tokenizer = BertTokenizer.from_pretrained(bert_checkpoint, subfolder=bert_subfolder_tokenizer)
...
class TextPipeline(nn.Module):
    ...
    def __init__(
        ...
        self.clip = CLIPTextEncoder(clip_checkpoint, subfolder_encoder=clip_subfolder_encoder, subfolder_tokenizer=clip_subfolder_tokenizer)

        if translation_matrix is None:
            translation_matrix = TranslationMatrix(
                clip_checkpoint, bert_checkpoint,
                clip_subfolder_tokenizer=clip_subfolder_tokenizer,
            ).translation_matrix
            ...
            translation_matrix_name = make_translation_matrix_name(
                clip_checkpoint, bert_checkpoint, clip_subfolder_encoder, clip_subfolder_tokenizer
            )
            print(f"Saving translation matrix to {checkpoints_dir / translation_matrix_name}")
            torch.save(
                translation_matrix,
                # checkpoints_dir / "translation_matrix.pt",
                checkpoints_dir / translation_matrix_name,
...
class TextPipelineWithProjection(nn.Module):
    """Predict masked tokens with BERT and translate them into CLIP embeddings."""
    ...
    def __init__(
        ...
        super().__init__()
        self.bert = BertMasked(checkpoint=bert_checkpoint, n_soft_prompt=n_soft_prompt)
        self.clip = CLIPTextEncoderWithProjection(clip_checkpoint, subfolder_encoder=clip_subfolder_encoder, subfolder_tokenizer=clip_subfolder_tokenizer)
    ...
    def forward(
        ...
        clip_emb = encoder_out.last_hidden_state
        text_embeds = encoder_out.text_embeds      # projected embedding (shape: [batch, projection_dim])
        ...
        return clip_emb, text_embeds, masked_loss, pred_word
