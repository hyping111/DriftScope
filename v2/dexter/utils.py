...
def load_translation_matrix(clip_checkpoint, bert_checkpoint, clip_subfolder_encoder, clip_subfolder_tokenizer):
    ...
    print(f"Loading translation matrix from {checkpoints_dir / translation_matrix_name}")
    return torch.load(checkpoints_dir / translation_matrix_name)
...
def text_enc_with_proj(prompts, tokenizer, text_encoder, maxlen=None, device="cuda"):
    """Tokenize prompts and run them through the provided text encoder.
    ...
    outputs = text_encoder(inp.input_ids.to(device))
...
def make_translation_matrix_name(clip_checkpoint, bert_checkpoint, clip_subfolder_encoder="", clip_subfolder_tokenizer=""):
    """Generate a unique filename for the translation matrix based on model checkpoints.

    Args:
        clip_checkpoint: CLIP model checkpoint identifier.
        bert_checkpoint: BERT model checkpoint identifier.
        clip_subfolder_encoder: Optional subfolder for CLIP encoder weights.
        clip_subfolder_tokenizer: Optional subfolder for CLIP tokenizer.
    
    Returns:
        A string filename for the translation matrix.
    """
    return f"translation-matrix_{clip_checkpoint.replace('/', '-')}{('-' + clip_subfolder_encoder) if clip_subfolder_encoder != '' else ''}{('-' + clip_subfolder_tokenizer) if clip_subfolder_tokenizer != '' else ''}_{bert_checkpoint.replace('/', '-')}.pt"
